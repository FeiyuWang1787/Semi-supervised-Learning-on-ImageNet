...
from tensorpack.utils.fs import download, get_dataset_path, mkdir_p
...
class ILSVRCMeta(object):
    ...
    def get_image_list(self, name, dir_structure='original', labeled=True, balanced=True):
        """
        Args:
        ...
        if labeled:
            ...
            fname = name + '_labeled.txt' if not balanced else name + '_labeled_balanced.txt'
        else:
            print("Read unlabeled training")
            fname = name + '_unlabeled.txt' if not balanced else name + '_unlabeled_balanced.txt'
...
class ILSVRC12Files(RNGDataFlow):
    ...
    def __init__(self, dir, name, meta_dir=None,
                 shuffle=None, dir_structure=None, labeled=True, balanced=True):
        """
        ...
        self.imglist = meta.get_image_list(name, dir_structure, labeled=labeled, balanced=balanced)
...
class ILSVRC12(ILSVRC12Files):
    ...
    def __init__(self, dir, name, meta_dir=None,
        ...
        super(ILSVRC12, self).__init__(
            dir, name, meta_dir, shuffle, dir_structure, labeled=labeled, balanced=balanced)
    """
